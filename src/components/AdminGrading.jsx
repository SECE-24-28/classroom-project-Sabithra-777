...
const AdminGrading = () => {
  ...
  const fetchAssignmentByName = async (assignmentName) => {
    setLoading(true);
    try {
      const response = await adminAPI.getAssignmentByName(assignmentName, admin.id);
      ...
      alert('Error: ' + (error.response?.data?.message || 'Assignment not found'));
      setSubmissions([]);
      setSelectedAssignment(null);
    } finally {
      setLoading(false);
    ...
    <div style={styles.container}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <div style={styles.card}>
          <h3 style={{ marginBottom: '1rem' }}>Enter Assignment Name to Grade</h3>
          <input
            type="text"
          ...
          <button
            onClick={() => {
              const input = document.querySelector('input[placeholder*="Assignment Name"]');
              if (input.value.trim()) {
                ...
                <div key={submission._id} style={styles.submissionCard}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                    <div>
                      <h4 style={{ margin: '0 0 0.5rem 0', color: '#1f2937' }}>
                        {submission.user?.firstName} {submission.user?.secondName}
                      </h4>
                      <p style={{ margin: '0 0 0.5rem 0', color: '#6b7280', fontSize: '0.9rem' }}>
                        Email: {submission.user?.email}
                      </p>
                      <p style={{ margin: '0 0 1rem 0', color: '#6b7280', fontSize: '0.9rem' }}>
                        Submitted: {new Date(submission.completedTime).toLocaleString()}
                      </p>
                      
                      {submission.filePath && (
                        <a
                          href={`http://localhost:21000/${submission.filePath}`}
                          target="_blank"
                  ...
                  <div style={styles.gradeForm}>
                    ...
                    <button
                      ...
                      style={{
                        ...styles.button,
                        background: gradingId === submission._id ? '#9ca3af' : 'linear-gradient(135deg, #10b981 0%, #059669 100%)',
                        cursor: gradingId === submission._id ? 'not-allowed' : 'pointer'
                      }}
                    >
